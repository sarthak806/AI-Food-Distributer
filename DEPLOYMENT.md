...
3. On the service's **Environment** page, add the secret values listed in `Backend/.env.example`, including the three Cloudinary values, plus `VITE_GOOGLE_CLIENT_ID` and `VITE_GOOGLE_MAPS_API_KEY` from `Frontend/.env.example`.
