...
3. On the service's **Environment** page, add the secret values listed in `Backend/.env.example`, including the three Cloudinary values, plus `VITE_GOOGLE_MAPS_API_KEY` from `Frontend/.env.example`.
4. Deploy. Render installs the frontend and backend dependencies, then applies the Render URL automatically while building the frontend.
5. Restrict the browser Google Maps key to the deployed URL.
